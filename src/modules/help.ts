...
composer.callbackQuery("helper", async (ctx) => {
  try {
    await ctx.editMessageText(
      ctx.t("help") +
        "\n\nTo approve members who are already in waiting list, upgrade to premium! Contact @Changer1_bot for information on pricing.",
      {
    ...
    "You can add me to channels as well as groups. Use the buttons below!",
    {
      reply_markup: new InlineKeyboard()
        .url(
          ...
          `https://t.me/${ctx.me.username}?start${channelOrGroup}=by_Gamechangertips&admin=invite_users+manage_chat`,
        )
